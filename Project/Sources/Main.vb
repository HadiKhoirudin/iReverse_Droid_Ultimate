...
Public Class Main
    ...
    Public Sub New()
        ...
        OFPExtractor.OPFWorkerMTK.WorkerSupportsCancellation = True
        OFPExtractor.OPFWorkerMTK.WorkerReportsProgress = True
        AddHandler OFPExtractor.OPFWorkerMTK.DoWork, AddressOf OFPExtractor.BruteKeyMtk
        AddHandler OFPExtractor.OPFWorkerMTK.RunWorkerCompleted, AddressOf OFPExtractor.AllDone
    ...
    Private Sub XtraMain_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        RichLogs("<+++++++++++++++++++  iREVERSE DROID ULTIMATE  ++++++++++++++++++++>", Color.Moccasin, True, True)
        RichLogs("► Software       : ", Color.Moccasin, True, False)
        RichLogs("Mediatek, Qualcomm, & Spreadtrum Tool", Color.Moccasin, True, True)
        RichLogs("► Version Tool   : ", Color.Moccasin, True, False)
        RichLogs("20-06-2023", Color.Moccasin, True, True)
        RichLogs("► License        : ", Color.Moccasin, True, False)
        RichLogs("Developer", Color.Moccasin, True, True)
        RichLogs("► Version Update : ", Color.Moccasin, True, False)
        RichLogs("Beta II based [ 13-09-2023 ] Version", Color.Moccasin, True, True)
        RichLogs("  ==================================================================", Color.Moccasin, True, True)
        RichLogs("► Websites       : https://hadikhoirudin.github.io/ireverse", Color.Moccasin, True, True)
        RichLogs("  ==================================================================", Color.Moccasin, True, True)
        RichLogs("", Color.Moccasin, True, True)
    ...
    Private Sub Main_Shown(sender As Object, e As EventArgs)
        ...
        If File.Exists(Mediatek.Mediatek_tool.Flashpath.Flashtoolexe) Then
            Dim directory2 As New DirectoryInfo(Path.GetDirectoryName(Mediatek.Mediatek_tool.Flashpath.Flashtoolexe) & "/")
        ...
        Dim directory As New DirectoryInfo(Windows.Forms.Application.StartupPath & "/Tools")
    ...
    Private Sub MenuOneClick(merk As String)
        ...
        QcMerkTerpilih = merk
        Try
            Dim webRequest As WebRequest = WebRequest.Create(String.Concat(datatool, merk.ToString().ToUpper(), ".txt"))
            ...
            Dim DataDevices As New ArrayList()
            Dim DataModels As New ArrayList()
    ...
    Public Shared Sub Flashtool_Doworker_DoWork(sender As Object, e As DoWorkEventArgs) Handles Flashtool_Doworker.DoWork
        Dim sevenZipPath As String = Path.Combine(Path.GetDirectoryName(Reflection.Assembly.GetExecutingAssembly().Location), If(Environment.Is64BitProcess, "Tools\x64", "Tools\x86"), "7z.dll")
        SevenZip.SevenZipBase.SetLibraryPath(sevenZipPath)
        Dim sevenZipExtractor As New SevenZip.SevenZipExtractor(Mediatek.Mediatek_tool.Flashtoolpath.DevExpres, "collabs0#0#")
    ...
    Private Sub RichTextBox1_TextChanged(sender As Object, e As EventArgs) Handles RichTextBoxLogs.TextChanged
        RichTextBoxLogs.Invoke(Sub()
    ...
    Public Shared Sub ButtonSTOP_Click(sender As Object, e As EventArgs) Handles ButtonSTOP.Click
        ...
        ElseIf Chipset = "Mediatek" Then
            If MtkFlash.SharedUI.SharedUI.BgwFlashfirmware.IsBusy OrElse MtkUnlock.SharedUI.SharedUI.BgwUnlocked.IsBusy Then
                ...
                KillCommand.ProcessKill()
            End If
            If OFPExtractor.OPFWorkerMTK.IsBusy Then
                RichLogs(" ", Color.Red, True, True)
                RichLogs("Button STOP clicked process will end! ", Color.Yellow, True, False)
                OFPExtractor.OPFWorkerMTK.CancelAsync()
                OFPExtractor.OPFWorkerMTK.Dispose()
            End If
        ElseIf Chipset = "Qualcomm" Then
        ...
        If FastbootUI.SharedUI.FastbootWorker.IsBusy Then
            ...
            KillCommand.ProcessKill()
        End If
        If DirectISP.SharedUI.DirectISPWorker.IsBusy Then
            RichLogs(" ", Color.Red, True, True)
            DirectISP.SharedUI.DirectISPWorker.CancelAsync()
            DirectISP.SharedUI.DirectISPWorker.Dispose()
        End If
        If DirectISP.SharedUI.eMMCISPWorker.IsBusy Then
            RichLogs(" ", Color.Red, True, True)
            RichLogs("Button STOP clicked process will end! ", Color.Yellow, True, False)
            DirectISP.SharedUI.eMMCISPWorker.CancelAsync()
            DirectISP.SharedUI.eMMCISPWorker.Dispose()
        End If
        If Unlock.SharedUI.WorkerUnlock.IsBusy Then
            RichLogs(" ", Color.Red, True, True)
            RichLogs("Button STOP clicked process will end! ", Color.Yellow, True, False)
            Unlock.SharedUI.WorkerUnlock.CancelAsync()
    ...
    Private Sub ButtonMiModel_Click(sender As Object, e As EventArgs) Handles ButtonMiModel.Click
        Brand = "XIAOMI"
        MainSelected = "unlock"
        If Not (FormCtrl.Controls.Contains(UnlockCtrl)) Then
            FormCtrl.Controls.Clear()
            FormCtrl.Controls.Add(UnlockCtrl)
            UnlockCtrl.BringToFront()
            UnlockCtrl.Dock = DockStyle.Fill
        End If
        Oneclick.Unlock_list.ListDevice.CreateListDevice()
    ...
    Private Sub ButtonFlashMtk_Click(sender As Object, e As EventArgs) Handles ButtonFlashMtk.Click, ButtonFlashSpd.Click
        PictureEditSelectedChip.EditValue = Global.Reverse_Tool.My.Resources.logochipmediatek
        Chipset = "Mediatek"
        ...
        FormCtrl.Controls.Clear()
        ButtonFlashApple.Enabled = True
        ButtonFlashMtk.Enabled = False
        ButtonFlashQc.Enabled = True
        ButtonFlashSpd.Enabled = True
        FormCtrl.Controls.Add(MtkFlashCtrl)
        MtkFlashCtrl.BringToFront()
        MtkFlashCtrl.Dock = DockStyle.Fill
    End Sub
    ...
    Private Sub labelControlFlash_Click(sender As Object, e As EventArgs) Handles labelControlFlash.Click
        MainSelected = "flash"
        If Chipset = "Qualcomm" Then
            If Not (FormCtrl.Controls.Contains(QcFlashCtrl)) Then
                FormCtrl.Controls.Clear()
            ...
            End If
        ElseIf Chipset = "Mediatek" Then
            If Not (FormCtrl.Controls.Contains(MtkFlashCtrl)) Then
                FormCtrl.Controls.Clear()
                FormCtrl.Controls.Add(MtkFlashCtrl)
                MtkFlashCtrl.BringToFront()
                MtkFlashCtrl.Dock = DockStyle.Fill
        ...
        End If
    End Sub
    Private Sub LabelControl2_Click(sender As Object, e As EventArgs) Handles labelControlUnlock.Click
        MainSelected = "unlock"
        If Not (FormCtrl.Controls.Contains(UnlockCtrl)) Then
            FormCtrl.Controls.Clear()
            FormCtrl.Controls.Add(UnlockCtrl)
    ...
    Public Sub LabelControlADBIdent_Click(sender As Object, e As EventArgs) Handles labelControlADBIdent.Click
        ...
        If comboUSB.Text.Contains("PHYSICALDRIVE") Then
            If DirectISP.SharedUI.DataView.Rows.Count > 0 Then
                ...
                EMMCISP.SelectedCommand = "identify"
                DirectISP.SharedUI.eMMCISPWorker.RunWorkerAsync()
                DirectISP.SharedUI.eMMCISPWorker.Dispose()
    ...
    Private Sub AdbDotNet_DoWork(sender As Object, e As DoWorkEventArgs) Handles AdbDotNet.DoWork
        AdbDotNetcmd = "ls"
        AdbTool.SharedUI.adbcmd(AdbDotNetcmd, AdbDotNet, e)
    End Sub
